...
function love.update(dt)
    ...
    elseif gameState == "play" then
        if ball:collides(player1) then
            ...
            ball.x = player1.x + 5 -- else infinte loop 

            if ball.dy < 0 then -- ball.dy will happen if ball is going in a straight path 
        ...
        if ball:collides(player2) then
            ball.dx = -ball.dx * 1.03
            ball.x = player2.x - 4 -- else infinte loop
        ...
        --score updation if ball reach beyond the screen
        --reset 
        if ball.x < 0 then
            servingPlayer = 1
